    ...
    def move(self):
        ...
        if possible_steps:
            ...
            for pos in possible_steps:
                ...
                stage_attraction = 20 * self.model.stage.desirability
...
class ConcertHall(mesa.Model):
    ...
    def create_bars(self):
        ...
        for x in range(bar_x - 2, bar_x + 3):
            self.grid.place_agent(self.bars[0], (x, 1))
    ...
    def create_exits(self):
        ...
        exit_bottom_start = 5
        for y in range(10):
            self.grid.place_agent(self.exits[0], (self.width - 2, exit_top_start + y))  # Place exits on the right wall
            self.grid.place_agent(self.exits[1], (self.width - 2, exit_bottom_start + y))
    ...
    def create_toilets(self):
        ...
        for x in range(toilet_x - 2, toilet_x + 3):
            self.grid.place_agent(self.toilets[0], (x, self.height - 2))
...
def run_single_agent_simulation(steps):
    ...
    for i in range(steps):
        model.step()
        agent = model.schedule.agents[0]  # Get the single agent
        print(f"Step {i+1}:")
